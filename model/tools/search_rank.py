...
def query_image_rank(image_path, file_scores):
    """
    查询给定图像在 file_scores 中的排名。
    """
    # 在 file_scores 中查找排名
    sorted_scores = sorted(file_scores, key=lambda x: -x[1])
    for rank, (file, score) in enumerate(sorted_scores, start=1):
        if image_path.endswith(file):
            return rank, score
    return -1, None  # 未找到匹配的图像
